...
var MemeBot = function Constructor(settings) {
    ...
    // if (!dbExists) {
    //     console.log('Database path ' + '"' + this.dbPath + '" does not exists or it\'s not readable, will create new.');
    // }
    ...
    //     if(!dbExists) {
    //         self.db.run("CREATE TABLE responses (post_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, cue TEXT NOT NULL, response TEXT NOT NULL)");
    //     }
    // });
};
...
MemeBot.prototype._isMentioningMemeBot = function (message) {
    return (isInMessage(message,'memebot')
        ...
        //&& message.text.toLowerCase().indexOf('me_irl') > -1;
};
